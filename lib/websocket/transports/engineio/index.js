...
module.exports = function(ss, messageEmitter, httpServer, config){
  ...
  ws.on('connection', function(socket) {
    ...
    if(socket.request.connection) {
		if(socket.request.connection.parser){
			if(socket.request.connection.parser.incoming){
				if(socket.request.connection.parser.incoming.headers){
					var xForwardedFor = socket.request.connection.parser.incoming.headers['x-forwarded-for'];
...
var processSession = function(ss, socket) {
  try {
    var rawCookie;
    if(socket.upgraded) {
    ...
    if(cookie){
    	if(cookie['connect.sid']){
	    	...
	    	socket.sessionId = ss.session.create();
	    }
    }
    return true;
  }
